...
module.exports = {
    ...
    callback: async(message, args) => {
        ...
        member.roles.add('830232694930538497')
        const time = args[1]
        if(!member) return message.reply('Mention A User to Temp Mute.')
        if(!time) return message.reply('Specify A Time To Mute')

        if(member.roles.cache.has('830232694930538497')) return message.reply('User Is Already Muted.') // If User Is Already muted.
        await member.roles.add('830232694930538497')
